...
Sort_Window::Sort_Window(QSqlDatabase* db, QWidget *parent):
    QWidget (parent),
...
void Sort_Window::display()
{
    select_recipes_from_db(*db, recipes);
    show_recipes<Ui::Sort_Window>(ui, recipes);
}
...
void Sort_Window::on_sort_Btn_clicked()
{
...
void Sort_Window::on_show_select_recipe_clicked()
{
    parent_window->show();
    this->hide();
}

void Sort_Window::on_show_add_recipe_page_btn_clicked()
{
    add_recipe->show();
    this->hide();
}
void Sort_Window::set_windows( AddRecipeWindow * add_recipe, MainWindow * parent_window)
{
